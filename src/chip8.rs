...
pub fn init(fontset: &[u8; 80], program: Vec<u8>, canvas: Canvas<Window>, events: sdl2::EventPump) -> Chip8 {
    let mut chip8 = Chip8 { opcode:      0,
                            mem:         [0; 4096],
                            regs:        [0; 16],
                            addr:        0,
                            pc:          0x200,
...
impl Chip8 {
    pub fn emu_cycle(&mut self, draw_scale: usize, speed: u32) {
        loop {
            ...
            self.handle_keys();
            if self.events.keyboard_state().is_scancode_pressed(Scancode::Escape) {
                println!("Quitting...");
                return;
            }
    ...
    fn draw_canvas(&mut self, draw_scale: usize) {
                ...
                if self.px_grid[x + y * 64] {
                    for x1 in 0..draw_scale {
                        for y1 in 0..draw_scale {
                            to_draw_white.push(Point::from(((x * draw_scale + x1) as i32, (y * draw_scale + y1) as i32)))
                        }
                    }
                } else {
                    for x1 in 0..draw_scale {
                        for y1 in 0..draw_scale {
                            to_draw_black.push(Point::from(((x * draw_scale + x1) as i32, (y * draw_scale + y1) as i32)))
                        }
                    }
                }
            }
        ...
        self.canvas.draw_points(&to_draw_white[..]).expect("Error drawing!");

        self.canvas.set_draw_color(Color::RGB(0, 0, 0));
        self.canvas.draw_points(&to_draw_black[..]).expect("Error drawing!");
            ...
            0xD000 => {
                ...
                for yline in 0..n {
                    ...
                    for xline in 0..8 {
                        ...
                        if color != 0 {
                            self.regs[0xF] |= (color & self.px_grid[x_pos + (y_pos * 64)] as u16) as u8;
                        }
